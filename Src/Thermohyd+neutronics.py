...
ah=-70e-5 #pcm/cm
h0=0
h=float(input("Insersci il valore di h (altezza delle barre di controllo, in cm): "))
BETA = 650e-5 
...
rho_c = 702.45 # kg/m³
m_dot_channel = flowrate / (2 * (num_ass * num_rod_ass))
...
rho= lambda TF,TM, H: af*(float(TF)-(T2_ss))+am*(float(TM)-(Tc_ss))+ah*(float(H)-h0)


#DEFINIZIONE DELL'EQUAZIONE DIFFERENZIALE PER ANDAMENTO TEMPERATURA FUEL E COOLANT

def fuel_thermal_dynamics(t, y, T_inlet):
    ...
    Flowr_input = np.where (t<30, flowrate, flowrate*1.1) # incremento del 10% del flowrate dopo 10 secondi
    T_input = np.full_like (t, 277.0+273.15)
    m_dot_channel = Flowr_input / (2 * num_rod_ass * num_ass)
    ...
    dndt  = ((rho(T2,Tc,h) - BETA) / PROMPT_LIFETIME) * n + np.dot(LAMBDAS, c)
    ...
    dTcdt  = (Qcl - 2 * m_dot_channel * cp_c * (Tc - T_input)) / (Mc * cp_c) # eventualmente cambiare T_inlet con T_input
    

    return [dT1dt, dT2dt, dT3dt, dTcldt, dTcdt , dndt, *dcidt]

n0 = 1.0
c0 = (BETAS_I / (LAMBDAS * PROMPT_LIFETIME)) * n0
y0 = [T1_ss, T2_ss, T3_ss, Tcl_ss, Tc_ss, n0, *c0]
sol = solve_ivp(fuel_thermal_dynamics, [0, 100], y0, args=(T_inlet,), method='LSODA')

#PLOT
#volendo si potrebbe plottare anche il cladding
fig, ax1 = plt.subplots(1, 2, figsize=(12, 8))
ax1[0].plot(sol.t, sol.y[0]-273.15, color='red', label="Centro Fuel")
ax1[0].axhline(y=T1_ss-273.15, color='black', linestyle='--', alpha=0.5, label='Stazionario ($T_{f0}$)')
ax1[0].set_title("Temperatura Centro del Combustibile con un incremento di Potenza del 10%")
...
fig, ax2 = plt.subplots(1, 2, figsize=(12, 8))
ax1[1].plot(sol.t, sol.y[4]-273.15, color='blue', label="Coolant")
ax1[1].axhline(y=Tc_ss-273.15, color='black', linestyle='--', alpha=0.5, label='Stazionario ($T_{c0}$)')
ax1[1].set_title("Temperatura Coolant con un incremento di Potenza del 10%")
ax1[1].set_xlabel("Tempo (s)")
ax1[1].set_ylabel("Temperatura (°C)")
ax1[1].grid(True, linestyle=':')
plt.legend()

ax2[0].plot(sol.t, sol.y[2]-273.15, color='orange', linewidth=2, label='Superficie Fuel ($T_f$)')
ax2[0].axhline(y=T3_ss-273.15, color='black', linestyle='--', alpha=0.5, label='Stazionario ($T_{f0}$)')
ax2[0].set_title("Temperatura Superficie del Combustibile con un incremento di Potenza del 10%")
ax2[0].set_xlabel("Tempo (s)")
ax2[0].set_ylabel("Temperatura (°C)")
ax2[0].grid(True, linestyle=':')
plt.legend()

ax2[1].plot(sol.t, sol.y[5], color='blue', label="Densutà Neutronica")
ax2[1].set_title("Densità Neutronica")
ax2[1].set_xlabel("Tempo (s)")
ax2[1].set_ylabel("PCM")
